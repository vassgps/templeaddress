...
const people = [
  {id:'U10084',name:'Jinsha K',type:'Agent',code:'JIN-4471',mobile:'+91 ••••••1196',email:'jinsha@example.com',auth:'Mobile OTP',verification:'KYC verified',dealer:'DLR-KZN-014',balance:6840,joined:'18 Mar 2026'},
  {id:'U10085',name:'Sangeetha P',type:'Agent',code:'SAN-0932',mobile:'+91 ••••••0932',email:'sangeetha@example.com',auth:'Google',verification:'KYC verified',dealer:'DLR-KZN-014',balance:2210,joined:'04 Apr 2026'},
  {id:'U10086',name:'Arun K',type:'Agent',code:'ARU-2210',mobile:'+91 ••••••2210',email:'arun@example.com',auth:'Mobile OTP',verification:'Under review',dealer:'DLR-KZE-009',balance:1480,joined:'16 May 2026'},
...
export function StaffDirectory(){
  ...
  const balance=person=>(person.balance||0)+records.reduce((total,record)=>total+(record.agentId===person.code?record.amount:record.dealerId===person.code?record.dealerAmount:0),0)
  ...
  async function post(event){
    ...
    const save=()=>{const existing=readRewards();if(existing.some(record=>record.campaign===code))throw new Error('Campaign ID already used. Enter a new one-time code.');const now=new Date();const transactionId=`WLT-${now.toISOString().slice(2,10).replaceAll('-','')}-${now.getTime().toString().slice(-6)}`;const entry={id:crypto.randomUUID(),transactionId,date:now.toISOString(),staffName:'Anand',staffId:'STF-1001',staffRole:role,agentId:profile.code,agentUserId:profile.id,agentName:profile.name,dealerId:profile.dealer,amount:value,dealerAmount:Math.round(value*10)/100,reason:reason.trim(),campaign:code,listing};const next=[entry,...existing];localStorage.setItem(rewardKey,JSON.stringify(next));setRecords(next)}
...
function UserModal({profile,role,balance,records,reward,setReward,form,onClose,onOpenTransaction}){
  ...
  return <Modal label="User details" onClose={onClose}><div className="pr-24"><p className="text-xs font-semibold uppercase tracking-wide text-saffron-600">User details</p><h2 className="mt-1 text-xl font-semibold">{profile.name} · {profile.code}</h2><div className="mt-2 flex flex-wrap gap-2"><Pill>{profile.type}</Pill><Pill tone={profile.verification.includes('pending')||profile.verification==='Under review'?'warn':'ok'}>{profile.verification}</Pill></div></div>
    ...
    {profile.balance!==undefined&&<section className="mt-6"><h3 className="font-semibold">Reward history</h3>{records.filter(record=>record.agentId===profile.code||record.dealerId===profile.code).map(record=><div key={record.id} className="mt-3 border-t p-3 text-sm"><b>{record.campaign} · {money(record.agentId===profile.code?record.amount:record.dealerAmount)}</b><p>Transaction ID: <b>{record.transactionId||`WLT-${String(record.id).slice(0,8).toUpperCase()}`}</b></p><p>{record.reason} · Listing {record.listing}</p><p>{new Date(record.date).toLocaleString()} · {record.staffName} · {record.staffId}</p></div>)}{!records.some(record=>record.agentId===profile.code||record.dealerId===profile.code)&&<p className="mt-2 text-sm text-brown-500">No reward transactions recorded.</p>}</section>}
  </Modal>
}

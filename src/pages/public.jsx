  ...
  return (<PublicShell tenant={wl?t.name:null}><Wrap className="py-6">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
      <div className="space-y-6">
        ...
        <div className="card p-5"><h2 className="text-xl font-semibold">Contact & directions</h2><p className="mt-1 text-sm text-brown-700">{t.address}</p><div className="mt-3 flex flex-wrap gap-2">{t.publicPhone&&<a href={`tel:${t.publicPhone}`} className="btn-g"><Phone size={16}/>Call office</a>}{t.publicEmail&&<a href={`mailto:${t.publicEmail}`} className="btn-g">Email office</a>}<a href={mapUrl} target="_blank" rel="noopener noreferrer" className="btn-g"><MapPin size={16}/>Open in Maps</a><a href={`https://wa.me/?text=${encodeURIComponent(`${t.name} ${pageUrl}`)}`} target="_blank" rel="noopener noreferrer" className="btn-g"><Share2 size={16}/>Share</a></div><iframe title={`Map of ${t.name}`} loading="lazy" referrerPolicy="no-referrer" src={`https://maps.google.com/maps?q=${encodeURIComponent(mapQuery)}&output=embed`} className="mt-4 h-64 w-full rounded-xl border-0"/><p className="mt-2 text-xs text-brown-500">{t.latitude == null ? 'Map searches the listing address; verify directions before travel.' : 'Map uses the listing coordinates.'}</p></div>
      ...
      <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
        ...
        <div className="card p-5"><div className="flex items-center gap-2"><QrCode size={18}/><b>Temple QR code</b></div><QRCodeSVG value={pageUrl} size={160} marginSize={4} className="mx-auto mt-3"/><p className="mt-2 break-all text-xs text-brown-500">{pageUrl}</p><p className="mt-1 text-xs text-brown-500">Scan to open {t.name} and book a pooja.</p></div>

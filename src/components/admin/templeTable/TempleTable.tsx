...
import TempleTr from "./templeTr/TempleTr";
...
const TempleTable = ({  search }) => {
  ...
  const [items, setItems] = useState<Temple[] | undefined>();
  ...
  return (
    <div className="w-full  mb-12  px-4 mx-auto mt-10">
      <Search route={"/admin/temples"} search={search} />
      ...
      {!loading ? (
        <>
          {items&&items.length>0 && (
            <div className=" relative flex flex-col min-w-0  mt-5 break-words w-[170vh]  overflow-x-auto overflow-y-auto bg-white mb-6 shadow-lg rounded ">
              ...
              <div className=" block overflow-x-auto overflow-y-auto ">
                <table className="items-center bg-transparent w-full border-collapse ">
                  ...
                  <tbody>
                    {items.map((item) => (
                      <TempleTr key={item.id} item={item} />
                    ))}
                  </tbody>
                </table>
              </div>
